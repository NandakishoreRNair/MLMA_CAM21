...
% Tests how well each model generalises to unseen subjects
% Subject-dependent: train and test on same subject (LOTO-CV)
% Subject-independent: train on 14 subjects, test on 1 (LOSO-CV)
% Change ground_to_test to run on different terrains
...
% load 4-phase data for all subjects
...
for subj_idx = 1:length(subjects)
    ...
    if ~all_exist
        fprintf('skipping %s - phase files missing\n', subject);
        continue;
    end
    ...
    try
        ...
        % only use first 5 trials
        common_trials = common_trials(1:min(5, length(common_trials)));

        % align rows across phases per trial
        ...
        % transition step = label at 85% is a transition locomotion class
        lbl_85  = lbl_s{4};
...
% per-subject accuracy storage [overall, steady, transition]
dep_lda  = nan(length(subjects), 3);
...
% subject-dependent evaluation (train and test on same subject)
fprintf('running subject-dependent CV...\n\n');
tic;
...
for subj_idx = 1:length(subjects)
    ...
    fprintf('%s | %d trials | %.1f sec\n', subject, num_folds_s, toc);
    ...
    if ~isempty(all_truth)
        ...
        for mi = 1:4
            ...
            pred  = preds{mi};
            acc   = [mean(pred==all_truth) ...
                     mean(pred(s_m)==all_truth(s_m)) ...
                     mean(pred(t_m)==all_truth(t_m))]*100;
            switch mi
                case 1, dep_lda(subj_idx,:) = acc;
                case 2, dep_svm(subj_idx,:) = acc;
                case 3, dep_rf(subj_idx,:)  = acc;
                case 4, dep_dbn(subj_idx,:) = acc;
            end
        end
...
fprintf('\nsubject-dependent done! (%.1f sec)\n\n', toc);

% combine all subject data into one pool for subject-independent evaluation
fprintf('running subject-independent CV...\n\n');
...
% leave one subject out - train on 14, test on 1
for subj_idx = 1:length(subjects)
    if ~valid_subj(subj_idx), continue; end
...
fprintf('\nsubject-independent done! (%.1f sec)\n\n', toc);
...
% plots
figure('Name','Dep vs Indep','Position',[100 100 1000 600]);
...
for col = 1:3
    ...
    legend({'Subject-Dependent','Subject-Independent'}, 'Location','southeast','FontSize',7);
    ylim([0 105]); grid on;
    for mi = 1:4
        text(mi-0.15, dep_vals(mi)+1, sprintf('%.1f',dep_vals(mi)), 'HorizontalAlignment','center','FontSize',7);
        text(mi+0.15, indep_vals(mi)+1, sprintf('%.1f',indep_vals(mi)), 'HorizontalAlignment','center','FontSize',7);
    end
end
...
fprintf(fid, 'subject-dependent vs subject-independent - %s\n', ground_to_test);
