...
%%   Final classification at 85% compared across models
%%
...
for subj_idx = 1:length(subjects)
    ...
    for fold = 1:num_folds_s
        ...
        % Normalise all phases using 85% training statistics
        ...
        for p = 1:num_phases-1
            ...
            for k = 1:length(Y_train{p})
                ...
                Tp(from_cls, to_cls) = Tp(from_cls, to_cls) + 1;
            end
            T{p} = Tp ./ sum(Tp, 2);
        end
        ...
        %% ---- LDA at 85% only (baseline — no temporal) ----
        if lda_ok(4)
        ...
        try
            ...
            %% Phase 2 (35%) — propagate from 10%, update with 35% emission
            ...
            %% Phase 3 (60%) — propagate from 35%, update with 60% emission
...
title(sprintf(['Steady State vs Transition Accuracy\n' ...
    '(%s | 4-phase DBN chain: 10%%->35%%->60%%->85%%)'], ground_to_test));
...
saveas(gcf, fullfile(output_folder, 'hypothesis_4phase.png'));
...
saveas(gcf, fullfile(output_folder, 'dbn_advantage_4phase.png'));

% Plot 3: Per-subject DBN vs LDA
...
saveas(gcf, fullfile(output_folder, 'per_subject_4phase.png'));
...
txt_out = fullfile(output_folder, 'hypothesis_4phase_results.txt');
fid = fopen(txt_out, 'w');
...
if trans_gap > 2 && steady_gap < 2
    ...
    fprintf(fid, '           Similar at steady state (%.2f%% max diff)\n', steady_gap);
elseif trans_gap > 0
