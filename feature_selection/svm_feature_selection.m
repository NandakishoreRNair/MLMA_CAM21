%% ========================================================
%% FEATURE SELECTION: SVM MODEL (Linear Kernel)
...
fprintf('  Model            : SVM (Linear Kernel, One-vs-One multiclass)\n');
...
%%   CV loop re-normalises per fold to avoid leakage.
%% ========================================================
...
%% ========================================================
%% STEP 3: SVM FEATURE SELECTION  (Leave-One-Trial-Out CV)
%% ========================================================

fprintf('=======================================================\n');
fprintf('STEP 3: SVM FEATURE SELECTION\n');
...
% SVM template — linear kernel, defined once for efficiency
svm_template = templateSVM('KernelFunction', 'linear', ...
                            'Standardize', false,'Solver','ISDA');  % we standardise manually per fold
...
while length(selected_features) < MAX_FEATURES_TARGET && ~isempty(remaining_features)
    ...
    for k = 1:length(remaining_features)
        ...
        X_subset = X_combined(:, current_features);   % raw data — normalise per fold
...
fprintf('\n[%s] SVM feature selection complete!\n', datetime('now','Format','HH:mm:ss'));
...
txt_filename = fullfile(output_folder, 'SVM_selected_features.txt');
fid = fopen(txt_filename, 'w');

fprintf(fid, '=======================================================\n');
fprintf(fid, 'SVM SELECTED FEATURES (Ground: %s)\n', ground_to_test);
fprintf(fid, '=======================================================\n\n');
fprintf(fid, 'Model  : SVM (Linear Kernel, ECOC One-vs-One)\n');
...
mat_filename = fullfile(output_folder, 'SVM_selected_features.mat');
...
fprintf('\nOutput saved to: %s\n', output_folder);
fprintf('\n=======================================================\n');
fprintf('SVM FEATURE SELECTION COMPLETE!\n');
fprintf('=======================================================\n');